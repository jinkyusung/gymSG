...
class LaneDetection:
    ...
    def edge_detection(self, gray_image):
        ...
        gradient_x = sobel(gray_image, axis=0, mode='reflect')
        gradient_y = sobel(gray_image, axis=1, mode='reflect')
        ...
        # plt.imshow(gradient_sum, cmap='gray')
        # plt.savefig('gradient_sum.png')
        return gradient_sum
    ...
    def lane_detection(self, state_image_full):
        ...
        if lane_found:
            ...
            def delete_element(target, x):
                return np.delete(target, np.where(np.isin(target, x)))

            def get_closest(x: int, y: int, target: List[np.ndarray], direct: str):
                to_find = target
                if direct == "upper":
                    to_find = target[y+1]
                elif direct == "lower":
                    if y == 0:
                        return float(inf)
                    to_find = target[y-1]
                differences = np.abs(to_find - x)
                closest_index = np.argmin(differences)
                closest = to_find[closest_index]
                return closest

            while True:
                curr_x1, curr_y1 = lane_boundary1_points[-1]

                upper_closest = get_closest(curr_x1, curr_y1, maxima, "upper")
                lower_closest = get_closest(curr_x1, curr_y1, maxima, "lower")

                print(upper_closest, lower_closest)
